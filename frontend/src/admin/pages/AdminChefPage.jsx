...
const AdminChefPage = () => {
    ...
    const handleStaffDelete = async (id) => {
        if (!window.confirm("Delete this chef profile?")) return;
        try {
        ...
        >
            ...
            <div className="space-y-2 mb-6">
                {order.items?.map((item, idx) => (
                    <div key={idx} className="flex items-center justify-between py-1 border-b border-slate-50 last:border-0 group/item">
                        <span className="text-sm font-bold text-slate-700 tracking-tight">{item.MenuItem?.name || 'Unknown Item'}</span>
            ...
            <div className="pt-4 flex items-center justify-between">
                ...
                {(order.status === 'Preparing' || order.status === 'Pending') ? (
                    ...
                    >
                        Mark Ready <CheckCircle2 size={14} />
                    </button>
                ) : (
                    <button 
    ...
    return (
        <div className="space-y-10 pb-20">
            ...
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                <div>
                    <h1 className="text-4xl font-black text-slate-900 tracking-tight">Chef Menu</h1>
                    <p className="text-sm font-medium text-slate-500 mt-2 uppercase tracking-widest flex items-center gap-2">
                        <Flame size={14} className="text-admin-primary" /> Live Order Tracking & Staff Management
                    </p>
                </div>
            ...
            {activeTab === 'orders' ? (
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 items-start">
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between px-2">
                            <h3 className="text-base font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-3">
                                <UtensilsCrossed className="text-admin-primary" size={20} /> Preparing
                            </h3>
                            <span className="px-3 py-1 bg-white border border-slate-100 rounded-lg text-xs font-black text-admin-primary shadow-sm">
                                {orders.filter(o => o.status === 'Preparing' || o.status === 'Pending').length} Orders
                            </span>
                        </div>
                        <div className="space-y-6 min-h-[400px] p-2 rounded-[3rem] bg-slate-50/50 border border-dashed border-slate-200">
                            <AnimatePresence mode='popLayout'>
                                {orders.filter(o => o.status === 'Preparing' || o.status === 'Pending').map(order => (
                            ...
                            {orders.filter(o => o.status === 'Preparing' || o.status === 'Pending').length === 0 && (
                                <div className="h-60 flex flex-col items-center justify-center text-slate-400 gap-4">
                                    ...
                                    <p className="text-xs font-bold uppercase tracking-widest opacity-40">All orders clear</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-center justify-between px-2">
                            <h3 className="text-base font-black text-slate-900 uppercase tracking-[0.2em] flex items-center gap-3">
                                <CheckCircle2 className="text-emerald-500" size={20} /> Ready to Serve
                            </h3>
                            <span className="px-3 py-1 bg-white border border-slate-100 rounded-lg text-xs font-black text-emerald-500 shadow-sm">
                                {orders.filter(o => o.status === 'Ready').length} Available
                            </span>
                        </div>
                        <div className="space-y-6 min-h-[400px] p-2 rounded-[3rem] bg-emerald-50/20 border border-dashed border-emerald-200/50">
                            <AnimatePresence mode='popLayout'>
                            ...
                            {orders.filter(o => o.status === 'Ready').length === 0 && (
                                <div className="h-60 flex flex-col items-center justify-center text-slate-400 gap-4">
                                    ...
                                    <p className="text-xs font-bold uppercase tracking-widest opacity-40">No items waiting</p>
                                </div>
                            )}
                        </div>
                    </div>
                            ...
                            >
                                <div className="flex items-start justify-between mb-6">
                                    ...
                                    <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
                                        ...
                                        <button onClick={() => handleStaffDelete(chef.id)} className="p-2 text-slate-400 hover:text-rose-500 transition-colors"><Trash2 size={16}/></button>
                                    </div>
                                </div>
