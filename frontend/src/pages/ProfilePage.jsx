    ...
    return (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
            ...
            <div className="lg:col-span-7">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-8 md:p-10 shadow-sm border border-slate-100">
                    <div className="flex items-center space-x-4 mb-8 pb-6 border-b border-gray-100">
                        ...
                        <div>
                            <h2 className="text-2xl font-playfair text-slate-900">Personal Information</h2>
                    ...
                    <form onSubmit={handleSave} className="space-y-5">
                        <div>
                            ...
                            <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full px-5 py-4 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 focus:bg-white transition-all font-medium" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-2">Email <span className="ml-2 text-xs text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">Not editable</span></label>
                            <input type="email" value={formData.email} disabled className="w-full px-5 py-4 rounded-xl border border-slate-200 bg-slate-100 text-slate-500 cursor-not-allowed font-medium" />
                        </div>
                        <div>
                        ...
                        <div className="pt-2">
                            <button type="submit" disabled={isSaving} className={`btn-primary py-4 px-6 sm:px-10 rounded-xl flex items-center justify-center space-x-2 whitespace-nowrap text-xs sm:text-sm ${isSaving ? 'opacity-70 cursor-wait' : ''}`}>
            ...
            <div className="lg:col-span-5 space-y-8">
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                    <h3 className="text-xl font-playfair text-slate-900 mb-6">Account Stats</h3>
                    <div className="grid grid-cols-2 gap-6">
                        <div className="bg-slate-50 rounded-2xl p-5 text-center">
                            ...
                            <p className="text-base font-bold text-slate-900">{user?.memberSince || 'N/A'}</p>
                        </div>
                        <div className="bg-slate-50 rounded-2xl p-5 text-center">
                            ...
                            <p className="text-base font-bold text-slate-900">{user?.totalOrders || 0}</p>
                        </div>
                    </div>
                </motion.div>
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                    <h3 className="text-xl font-playfair text-slate-900 mb-6">Quick Actions</h3>
                    <div className="space-y-3">
                        {[
                            ...
                            <button key={i} onClick={() => window.history.pushState(null, '', `/profile?tab=${a.tab}`) || window.dispatchEvent(new PopStateEvent('popstate'))} className="w-full flex items-center space-x-4 p-4 rounded-2xl hover:bg-slate-50 transition-colors group text-left">
                                ...
                                <div className="flex-1">
                                    <p className="font-semibold text-slate-900 text-sm group-hover:text-primary transition-colors">{a.label}</p>
                        ...
                        <div key={order.id} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                                ...
                                <div className="flex items-center space-x-4">
                                    ...
                                    <button onClick={() => setSelectedOrder(order)} className="flex items-center space-x-1 text-primary hover:text-slate-900 text-sm font-semibold transition-colors">
...
const OrderDetail = ({ order, waiters = [], onBack }) => {
    ...
    return (
        <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }}>
            <button onClick={onBack} className="flex items-center space-x-2 text-slate-500 hover:text-primary transition-colors mb-8">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                <div className="lg:col-span-7 space-y-6">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                        <div className="flex items-center justify-between mb-6">
                            <h3 className="text-xl font-playfair text-slate-900">Order Status</h3>
                            <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColor(order.status)}`}>{order.status}</span>
                        </div>
                    ...
                    {/* Order Items */}
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                        <h3 className="text-xl font-playfair text-slate-900 mb-6">Order Items</h3>
                        <div className="space-y-4">
                ...
                <div className="lg:col-span-5 space-y-6">
                    {/* Contact Details */}
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                        <h3 className="text-xl font-playfair text-slate-900 mb-6">Contact Details</h3>
                        <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100">
                        ...
                        <div className="flex flex-col sm:flex-row gap-3">
                            ...
                            <button onClick={onBack} className="flex-1 py-3 rounded-xl border border-gray-200 text-slate-900 hover:border-primary hover:text-primary transition-colors text-sm font-semibold">
                                View All Orders
                            </button>
                        </div>
                    </div>
...
const AddressesTab = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="min-h-[500px]">
            <div className="flex items-center justify-between mb-8">
                <div>
                    <h2 className="text-2xl font-playfair text-slate-900">Saved Addresses</h2>
                    <p className="text-sm text-gray-500 mt-1">Manage your delivery addresses</p>
                </div>
                {!error && !isLoading && (
                    <button onClick={openAdd} className="btn-primary py-3 px-4 sm:px-6 rounded-xl flex items-center space-x-2 text-[10px] sm:text-sm whitespace-nowrap">
            ...
            {showForm && (
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-3xl p-8 shadow-sm border border-gray-100 mb-8">
                    <div className="flex items-center justify-between mb-6">
                        <h3 className="text-xl font-playfair text-slate-900">{editId ? 'Edit Address' : 'Add New Address'}</h3>
                    ...
                    <form onSubmit={handleSave} className="space-y-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                            <div>
                                ...
                                <input required value={formData.title} onChange={(e) => setFormData({ ...formData, title: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" placeholder="e.g. Home" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Address Type</label>
                                <select value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium">
                        ...
                        <div>
                            ...
                            <input required value={formData.line1} onChange={(e) => setFormData({ ...formData, line1: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" placeholder="Street address" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Address Line 2</label>
                            <input value={formData.line2} onChange={(e) => setFormData({ ...formData, line2: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" placeholder="Flat, suite, unit" />
                        </div>
                        <div>
                            <label className="block text-sm font-medium text-slate-700 mb-1">Landmark</label>
                            <input value={formData.landmark} onChange={(e) => setFormData({ ...formData, landmark: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" placeholder="Near..." />
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-5">
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">City *</label>
                                <input required value={formData.city} onChange={(e) => setFormData({ ...formData, city: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">State</label>
                                <input value={formData.state} onChange={(e) => setFormData({ ...formData, state: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" />
                            </div>
                            <div>
                                <label className="block text-sm font-medium text-slate-700 mb-1">Zip Code *</label>
                                <input required value={formData.zip} onChange={(e) => setFormData({ ...formData, zip: e.target.value })} className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-primary/50 bg-slate-50 font-medium" />
                            </div>
                        </div>
                        <div className="flex items-center justify-end space-x-4 pt-2">
                            <button type="button" onClick={() => setShowForm(false)} className="py-3 px-6 sm:px-8 rounded-xl border border-slate-200 text-slate-900 hover:border-primary hover:text-primary transition-colors font-bold text-[11px] sm:text-sm whitespace-nowrap">Cancel</button>
                            <button type="submit" className="btn-primary py-3 px-6 sm:px-8 rounded-xl text-[11px] sm:text-sm whitespace-nowrap">Save Address</button>
                        </div>
                    </form>
                </motion.div>
            )}
...
const ReservationsTab = () => {
    ...
    return (
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
                <div>
                    <h2 className="text-2xl font-playfair text-slate-900">My Reservations</h2>
                    <p className="text-sm text-gray-500 mt-1">Manage your table bookings</p>
                </div>
                {!error && !isLoading && (
                    <button onClick={() => navigate('/book')} className="btn-primary py-3 px-4 sm:px-6 rounded-xl flex items-center space-x-2 text-[10px] sm:text-sm whitespace-nowrap">
        ...
        <div className="min-h-screen bg-slate-50 pt-28 pb-24">
            <div className="container mx-auto px-6 max-w-6xl">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8">
                    <div>
                        <h1 className="text-3xl md:text-4xl font-playfair text-slate-900">Welcome, <span className="text-primary italic">{user.name?.split(' ')[0]}</span></h1>
                        <p className="text-slate-500 mt-1 text-sm">Manage your account</p>
                    </div>
                    <button onClick={handleLogout} className="mt-4 sm:mt-0 flex items-center space-x-2 text-red-500 hover:text-red-700 font-semibold transition-colors bg-red-50 hover:bg-red-100 px-5 py-2.5 rounded-xl text-xs sm:text-sm whitespace-nowrap">
